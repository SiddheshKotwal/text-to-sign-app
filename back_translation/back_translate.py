...
def build_vocab(field: str, vocab_file: str = None) -> Vocabulary:
    """
...
def load_checkpoint(path: str, use_cuda: bool = True) -> dict:
    ...
    # --- FIX: Force CPU loading for Streamlit app ---
...
def make_back_translation_model(model_dir: Union[str, Path]):
    ...
    use_cuda = False # cfg["training"].get("use_cuda", False)
    device = torch.device("cpu")
    ...
    gls_vocab = build_vocab(field="gls", vocab_file=gls_vocab_file)
    txt_vocab = build_vocab(field="txt", vocab_file=txt_vocab_file)
...
def back_translate(model: SignModel, poses: torch.Tensor) -> List:
    """
    ...
    sgn = [
        torch.cat((s, torch.zeros((max_len - s.shape[0], features_dim)))) for s in sgn
    ]  # pad
    ...
    with torch.no_grad():
        ...
        for i in tqdm(
            range(num_batches), desc="Running Back Translation", total=num_batches
        ):
            _sgn = sgn[start:end]
